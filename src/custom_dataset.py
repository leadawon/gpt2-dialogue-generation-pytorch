    ...
    def __init__(self, prefix, args):
        ...
        for dial in tqdm(dials):
            ...
            if len(input_ids) <= args.max_len: #token_ids와 label도 sp1 sp2 sp1 ... -100 -100 ... 적절하게 붙여줍니다.
                ...
                token_type_ids = [start_sp_id] + list(chain.from_iterable(token_type_ids)) + [args.sp1_id]
                assert len(input_ids) == len(token_type_ids)
                                
                labels = [[-100] * len(ctx) if c < len(hists)-1 else [-100] + ctx[1:] for c, ctx in enumerate(hists)]
                assert labels[-1][1:] == hists[-1][1:]
                labels = [-100] + list(chain.from_iterable(labels)) + [args.eos_id]
                assert len(input_ids) == len(labels)
